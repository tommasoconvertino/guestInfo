...
theme.LibraryLoader = (function() {
  ...
  var cloudCdn = 'https://web.archive.org/web/20230515160438/https://cdn.shopify.com/shopifycloud/';
...
theme.Video = (function() {
  ...
  function init(video) {
    ...
    if (!youtubeLoaded) {
      ...
      var tag = document.createElement('script');
      tag.src = 'https://web.archive.org/web/20230515160438/https://www.youtube.com/iframe_api';
      ...
      if (apiStatus !== 'loading') {
        ...
        if (typeof window.google === 'undefined') {
          theme.Helpers.getScript(
            'https://web.archive.org/web/20230515160438/https://maps.googleapis.com/maps/api/js?key=' + this.key
          ).then(function() {
            apiStatus = 'loaded';
            initAllMaps();
          });
...
/*CUSTOM COLOR CHANGE THINGY*/


function my_Change_clr(m_clr){
	document.getElementsByClassName("custColrS")[0].style.color = m_clr;
  	document.getElementsByClassName("custColrS")[1].style.color = m_clr;
  	

}

async function customColorChg() {
var style_img_str = "https://web.archive.org/web/20230515160438/https://cdn.shopify.com/s/files/1/0595/9122/9600/t/1/assets/prodS_";
  var my_first_time = 0;
  while(1){

  console.log("Starting NOWWW");
  my_Change_clr("black");
  document.getElementById("styleImgJS").setAttribute("src", style_img_str+ "black" +".png");
  if(!my_first_time)
    await new Promise(r => setTimeout(r, 600));
    else    await new Promise(r => setTimeout(r, 1290));
    my_first_time++;
  my_Change_clr("#35D3F4");
  document.getElementById("styleImgJS").setAttribute("src", style_img_str+ "blue" +".png");
  await new Promise(r => setTimeout(r, 1290));
  my_Change_clr("#858585");
  document.getElementById("styleImgJS").setAttribute("src", style_img_str+ "grey" +".png");
  await new Promise(r => setTimeout(r, 1290));
  my_Change_clr("#F52D53");
  document.getElementById("styleImgJS").setAttribute("src", style_img_str+ "red" +".png");
  await new Promise(r => setTimeout(r, 1290));
  my_Change_clr("#E9E2D9");
  document.getElementById("styleImgJS").setAttribute("src", style_img_str+ "white" +".png");
  await new Promise(r => setTimeout(r, 1290));
  my_Change_clr("#F6EE26");
  document.getElementById("styleImgJS").setAttribute("src", style_img_str+ "yellow" +".png");
      await new Promise(r => setTimeout(r, 1290));
  }
}

	let obs_elem = document.querySelector(".my-showcase");
  var only_once =0;
	let observer = new IntersectionObserver((entries, observer) => { 
		entries.forEach(entry => {
    			if(entry.intersectionRatio>.22){
				if(!only_once){
        only_once++;
       customColorChg();
        }
...
/*
     FILE ARCHIVED ON 16:04:38 May 15, 2023 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 16:29:22 Sep 14, 2023.
...
/*
playback timings (ms):
  captures_list: 1223.203
  exclusion.robots: 0.07
  exclusion.robots.policy: 0.061
  cdx.remote: 0.058
  esindex: 0.01
  LoadShardBlock: 100.233 (3)
  PetaboxLoader3.datanode: 62.997 (4)
  PetaboxLoader3.resolve: 947.75 (2)
  load_resource: 935.529
*/